...
module.exports = {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.commands.add("atom-workspace", {"run-in-terminal:OpenCommands.json": () => this.OpenCommands()}));
  },
  ...
  OpenCommands() {
    const filePath = path.join(__dirname, "commands.json");; // Change this path
    atom.workspace.open(filePath).catch((err) => console.error("Failed to open file:", err));
  ...
  consumeStatusBar(statusBar) {
	...
	terminalTest(){
    this.terminalService.run([
       'echo "Holy Crap This Actually Works?"'
    ])
  },
